...
import type { st_bazli, Ambalaj, Coordinate, Truck, TruckCapacity } from "../../types";
import { fetchTrucks, fetchTruckCapacities } from "../../api/trucksApi";
...
export function StatisticsPage({ st_bazliData, ambalajData, coordinates }: StatisticsPageProps) {
  ...
  const [truckCapacities, setTruckCapacities] = useState<TruckCapacity[]>([]);
  const [expandedTrucks, setExpandedTrucks] = useState<number[]>([]);

  const handleExpandAll = () => setExpandedTrucks(trucks.map(t => t.id));
  const handleCollapseAll = () => setExpandedTrucks([]);
  const toggleTruckExpand = (id: number) => {
    setExpandedTrucks(prev => prev.includes(id) ? prev.filter(tId => tId !== id) : [...prev, id]);
  };

  useEffect(() => {
    fetchTrucks().then(setTrucks).catch(console.error);
    fetchTruckCapacities().then(setTruckCapacities).catch(console.error);
  }, []);
  ...
  return (
    <div className="stats-page" id="statistics-page">
      ...
      <div className="stats__card" style={{ padding: "20px", marginBottom: "20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
          <h3 style={{ margin: 0, fontSize: "1.125rem", color: "#f8fafc" }}>Truck Fleet Overview</h3>
          <div style={{ display: "flex", gap: "8px" }}>
            <button 
              onClick={handleExpandAll}
              style={{ background: "#334155", color: "#f8fafc", border: "1px solid #475569", padding: "6px 12px", borderRadius: "6px", fontSize: "0.75rem", cursor: "pointer", fontWeight: "600", transition: "background 0.2s" }}
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#475569"}
              onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "#334155"}
            >Expand All</button>
            <button 
              onClick={handleCollapseAll}
              style={{ background: "transparent", color: "#f8fafc", border: "1px solid #475569", padding: "6px 12px", borderRadius: "6px", fontSize: "0.75rem", cursor: "pointer", fontWeight: "600", transition: "background 0.2s" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
          {trucks.map(truck => {
            const capacityInfo = truckCapacities.find(c => c.truck_id === truck.id);
            const percent = capacityInfo ? capacityInfo.Palet_percent : 0;
            const progressColor = percent > 90 ? "#ef4444" : percent > 70 ? "#f59e0b" : "#10b981";
            const isExpanded = expandedTrucks.includes(truck.id);

            return (
              <div key={truck.id} style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: "10px", padding: "16px", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.1)" }}>
                <div 
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <span style={{ fontSize: "1.1rem", fontWeight: "bold", color: "#e2e8f0" }}>{truck.Plaka}</span>
                    <span style={{ fontSize: "0.75rem", padding: "4px 8px", borderRadius: "6px", background: "rgba(99, 102, 241, 0.15)", color: "#818cf8", textTransform: "capitalize", fontWeight: "600" }}>{truck?.status?.replace("_", " ")}</span>
                  </div>
                  <span style={{ color: "#94a3b8", transform: isExpanded ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>▼</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "10px", fontSize: "0.875rem", color: "#94a3b8" }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span>Driver:</span>
                    <span style={{ color: "#f8fafc", fontWeight: "500" }}>{truck.Driver}</span>
                  </div>
                  ...
                  {isExpanded && (
                    <>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ color: "#f8fafc", fontWeight: "500" }}>{truck.Default_Sales_Rep || "N/A"}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Capacity (Palets):</span>
                        <span style={{ color: "#f8fafc", fontWeight: "500" }}>{truck.Capacity.toLocaleString()}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Current Miktar:</span>
                        <span style={{ color: "#f8fafc", fontWeight: "500" }}>{truck.Miktar.toLocaleString()}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Current Palet:</span>
                        <span style={{ color: "#f8fafc", fontWeight: "500" }}>{truck.Palet.toLocaleString()}</span>
                      </div>
                    </>
                  )}
                  ...
                  <div style={{ marginTop: "6px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px", fontSize: "0.75rem" }}>
                      <span>Usage</span>
                      <span style={{ color: progressColor, fontWeight: "bold" }}>{percent.toFixed(1)}%</span>
                    </div>
